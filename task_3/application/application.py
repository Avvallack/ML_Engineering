...
if __name__ == "__main__":
    ...
    with mlflow.start_run():
        ...
        grid = GridSearchCV(lr, param_grid, scoring='r2')
        grid.fit(train_x, train_y)

        predicted_qualities = grid.best_estimator_.predict(test_x)

        (rmse, mae, r2) = eval_metrics(test_y, predicted_qualities)

        print("Elasticnet model (alpha=%f, l1_ratio=%f):" % (grid.best_params_['alpha'], grid.best_params_['l1_ratio']))
        print("  RMSE: %s" % rmse)
        print("  MAE: %s" % mae)
        print("  R2: %s" % r2)
